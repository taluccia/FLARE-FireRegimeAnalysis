...
```{r}
data %>% group_by(FireYr, EcoName) %>%
  summarise(TotHa = sum(SizeHa))%>% as.data.frame(.) %>%
  summarise(total=sum(TotHa))
```
...
Table2 = data %>% group_by(EcoName) %>%
  summarise(avgHa = mean(round(SizeHa, 1)), 
            SD = sd(round(SizeHa, 1)),
            n = n(),
            se = SD / sqrt(n), 
            maxHa = max(SizeHa)) %>% 
  as.data.frame(.) %>%
...
```{r}
data1 = data %>% group_by(FireYr, EcoName, EcoCode, BTCodes) %>%
  summarise(TotHa = sum(SizeHa)) %>% 
  mutate(MHa=TotHa/1000000) %>%
  as.data.frame(.)
data1
...
fig1 = ggplot(data1, aes(x=FireYr, y=TotHa, fill=BTCodes)) +
...
```{r fig.height=6, fig.width=6}
fig1eco = ggplot(data1, aes(x=FireYr, y=TotHa, fill=EcoCode)) +
  geom_bar(stat="identity", width = 0.5, position = position_stack(reverse = FALSE)) +
  scale_y_continuous(name = ('Area burned \n(millions of hectares)'), breaks=c(0, 5000000, 10000000, 15000000, 20000000, 25000000, 30000000), labels = c(0, 5, 10, 15, 20, 25, 30)) +
...
```{r}
ggsave("../figures/1.1-Summary/Figure1eco.png", plot = fig1eco, width = 6, height = 5, units = c("in"), dpi = 600)
```
...
```{r}
data3 = data %>% dplyr::select(FireYr, SizeHa, EcoName, EcoCode, BTCodes, Class) %>%
  group_by(FireYr, EcoName, Class, EcoCode, BTCodes) %>%
  summarise(totha = sum(SizeHa)) %>%
  as.data.frame(.) %>%
...
fig3 = ggplot(data3, aes(x=FireYr, y=milha, fill=Class)) +
  geom_bar(stat="identity", width = 0.5, position = position_stack(reverse = FALSE)) +
  facet_wrap(~BTCodes, scales = "free_y", ncol=2) +
...
```{r}
data4 = data %>%
  dplyr::select(FireYr,EcoName, EcoCode, BTCodes, MinDay, MaxDay, AvgDay) %>%
  group_by(FireYr, EcoName, EcoCode, BTCodes) %>%
...
fig4.12 = ggplot(data4, aes(x=FireYr)) + 
  geom_bar(aes(y=season), stat="identity", fill="#003f5c", alpha=0.4, width = 0.5, position = position_stack(reverse = FALSE)) +
  geom_line(aes(y=start,  color="start"), size=1) +
  geom_line(aes(y=end,  color="end"), size=1) +
  geom_hline(yintercept=182, color = "black", linetype = "longdash") +
   scale_color_manual(values = c("start"="#FF6347","end"="#009CB8"), labels = c("Start", "End")) +
  facet_wrap(~EcoCode, scales = "free_y", ncol=8) +
  xlab('') +
  ylab('Julian date') +
  scale_y_continuous(name = "Julian date",
    sec.axis = sec_axis(trans = ~.*1, name="Season length (days)")) +
...
```{r}
ggsave("../figures/1.1-Summary/Figure4.12.png", plot = fig4.12, width = 24, height = 3, units = c("in"), dpi = 600)
```
## Figure 5---Start/End
...
data %>% 
  ...
  group_by(FireYr, EcoName, EcoCode) %>%
  count(FireYr, name='count') %>%
  filter(EcoCode %in% c("CKMT", "NST")) %>%
  filter(FireYr %in% c(2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019)) %>%
  ungroup() %>%
  summarise(total = sum(count))
```
...
```{r}
data10 = data %>% 
  dplyr::select(FireYr, SizeKm2, EcoName, EcoCode, Ecokm2, BTCodes) %>%
  group_by(FireYr, EcoName, EcoCode, BTCodes) %>%
  count(FireYr, name='count') %>%
  as.data.frame(.)
...
```{r fig.height=6, fig.width=6}
fig10 = ggplot(data10, aes(x=FireYr, y=count, fill=BTCodes)) +
  geom_bar(stat="identity", width = 0.5, position = position_stack(reverse = FALSE)) +
...
```{r}
data15 = data %>% 
  dplyr::select(FireYr, SizeHa, EcoName, EcoCode, BTCodes, Class) %>%
  group_by(FireYr, EcoName, Class, EcoCode, BTCodes) %>%
  count(FireYr, name='count') %>%
  as.data.frame(.) 
...
plot2 = cowplot::plot_grid(fig3, fig15,  ncol=2) +
    draw_label("(a)", x = .03, y = .975, size = 10) +
   draw_label("(b)", x = .53, y = .975, size = 10) 
