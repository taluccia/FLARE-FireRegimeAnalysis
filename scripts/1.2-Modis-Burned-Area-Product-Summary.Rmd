...
```{r}
target_eco = c("BT","CKMT", "CPT", "EST", "NSCT", "NST", "TBBMT", "TCST")
```

```{r}
all_data1 = all_data %>% filter(eco_code %in% target_eco)
...
```{r}
by_eco_yr = all_data1 %>% group_by(eco_code, fire_yr) %>% 
  summarise( . ,
...
```{r}
by_eco_yr %>% 
  rename(EcoCode = eco_code, FireYr = fire_yr, ModisMean = Mean, ModisSd = SD, ModisMedian = Median, ModisMin = Minimum, ModisMax=Maximum) %>% 
        write.csv(., '../outputs/ModisSummary/ModisSummary.csv',  row.names = FALSE)
