...
```{r}
fit1 = lm(sqrt(MHa) ~ SummerDef + EcoCode + SummerDef:EcoCode, data = climate)
```
#### Residuals
...
```{r}
g1 = ggplot(data = climate, aes(x = SummerDef, y = sqrt(MHa), color = EcoCode)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=("Annual Area burned in millions of Ha\n(Squareroot transformation)"), x= "CWD", color = "EcoCode") +
  theme_bw() + 
...
```{r}
fit2 = lm(sqrt(MHa) ~ SummerPr + EcoCode + SummerPr:EcoCode, data = climate)
```
#### Residuals
...
```{r}
g2 = ggplot(data = climate, aes(x = SummerPr, y = sqrt(MHa), color = EcoCode)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=("Annual Area burned in millions of Ha\n(Squareroot transformation)"), x= "precipitation", color = "EcoCode") +
  theme_bw() + 
  theme(legend.position = "bottom") +
  guides(colour = guide_legend(nrow = 1))
...
```{r}
g3 = ggplot(data = climate, aes(x = SummerTmmx, y = sqrt(MHa), color = EcoCode)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
