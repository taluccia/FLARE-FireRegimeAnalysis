...
```{r}
g4 = ggplot(data = data1, aes(x = season, y = sqrt(MHa), color = EcoCode)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=("Annual Area burned in millions of Ha\n(Squareroot transformation)"), x= "Summer Temperature", color = "EcoCode") +
  theme_bw() + 
...
```{r}
fit1 = lm(sqrt(Mha) ~ MeanCalDoY + EcoCode + MeanCalDoY:EcoCode, data = snow)
```
#### Residuals
...
```{r}
g5 = ggplot(data = snow, aes(x = MeanCalDoY, y = sqrt(Mha), color = EcoCode)) + 
  geom_smooth(method="lm", se=TRUE, alpha=0.1) +
  geom_point(size = 1) +
  scale_color_manual(values=cbbPalette) +
    labs(y=("Annual Area burned in millions of Ha\n(Squareroot transformation)"), x= "Snowmelt (Julian date)", color = "EcoCode") +
  theme_bw() + 
  theme(legend.position = "bottom") +
  guides(colour = guide_legend(nrow = 1))
